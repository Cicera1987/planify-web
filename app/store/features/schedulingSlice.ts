...
import type { Scheduling, SchedulingStatus } from "@/app/services/schedulingService"
...
export const fetchSchedulings = createAsyncThunk("scheduling/fetchSchedulings", async (search: string) => {
  const res = await api.get<Scheduling[]>(`/scheduling/search?name=${encodeURIComponent(search)}`)
  return res.data
})
...
const schedulingSlice = createSlice({
  ...
  reducers: {
    ...
    setImageState: (state, action: PayloadAction<Partial<SchedulingState["imageState"]>>) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateSchedulingStatus.fulfilled, (state, action) => {
        ...
        state.schedulings = state.schedulings.map((scheduling) => (scheduling.id === updated.id ? updated : scheduling))
      })
  },
})
